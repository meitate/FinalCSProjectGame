...
def spawn_monster(room, level):
    match level: # there are three levels of monsters 1 is the easist to defeat, 2 medium, 3 hard 
        case 1:
            index = randint(0,6)
...
def random_spawn_monster(room):
    chance = randint(0, 10) 
    if chance == 10: # 10% chance of a level 3 monster being in a room
        spawn_monster(room,3)
    if chance < 10 and chance > 6: # 30% chance of a level 2 monster being in a room
        spawn_monster(room,2)
    if chance <= 6 and chance >= 2: # 50% chance of a level 2 monster being in a room
        spawn_monster(room,2) 
    else: # 10% chance of no monster in a room
        return