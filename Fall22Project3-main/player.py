...
class Player:
    ...
    def get_item_by_name(self, name):
        ...
        for j in self.equipped:
            if j.name.lower() == name.lower():
                return j
        return False
    def get_item_by_name_equipped(self, name):
        for j in self.equipped:
            if j.name.lower() == name.lower():
                return j
        return False
    def check_weapon_equipped(self):
        for j in self.equipped:
            if isinstance(j, Weapon):
                return True
        return False
    ...
    def show_inventory(self):
        ...
        for i in self.items:
            print(Fore.CYAN + i.name + Style.RESET_ALL)
    ...
    def lose_health(self, damage):
        ...
        if self.health <= 0:
            self.health = 0
            self.alive = False
            print(f"{self.name} has died! Goodbye!")
            input("Press Enter")
            return False
        else:
                    ...
                    match decision:
                        case 1:
                            ...
                            if combat == True:
                                ...
                                self.money += randint(1,4) #Gives 1-3 coins for win
                                print (f"You now have {self.money} coins.")
    ...
    def monster_during_rest(self):
        ...
        monster = spawn_monster(location,1) #spawns a weak monster to attack the payer in their sleep
